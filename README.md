# STM32F7 USB Implementation

> USB CDC implementation for STM32F7 microcontroller with macOS compatibility

This project implements USB functionality for the STM32F7 microcontroller, specifically designed for reliable detection and communication with macOS systems.

## Gyro Functionality

The project includes a speed-optimized calibrated gyroscope system that provides relative angle measurements:

### Features

- **Auto-calibration on startup**: The gyro automatically calibrates when initialized, setting the current position as the zero reference point
- **Relative angle output**: Instead of raw sensor data, the system outputs Roll, Pitch, and Yaw angles relative to the startup position
- **Complementary filtering**: Combines gyroscope and accelerometer data for stable angle measurements
- **Real-time updates**: Continuous integration of sensor data to track orientation changes
- **Speed optimized**: Minimal processing overhead for maximum update rate

### Usage

1. **Power on**: The gyro initializes and calibrates automatically on startup (fast calibration with 10 samples)
2. **Zero reference**: The position at startup becomes 0° for all axes
3. **Tilt detection**: When you tilt the device 40° from the startup position, it will show "Roll: 40.0°"
4. **Return to zero**: When you return to the original position, it shows "Roll: 0.0°" again

### Output Format

The system outputs data in this simplified format:
```
Roll:0.0° Pitch:0.0° Yaw:0.0°
```

Where:
- **Roll/Pitch/Yaw**: Relative angles in degrees from calibration point

**Note**: Command interface removed for maximum speed - calibration only happens at startup.
